  ...
  template: `
    <div class="fixed bottom-6 right-6 z-[100] flex flex-col gap-2 pointer-events-none">
      @for (toast of toastService.toasts(); track toast.id) {
        <div
          class="pointer-events-auto flex items-start gap-3 px-4 py-3 rounded-xl border shadow-xl
                 backdrop-blur-sm min-w-72 max-w-sm animate-in slide-in-from-right-5 fade-in duration-300"
        ...
        >
          ...
          <p class="text-sm flex-1 text-slate-200 leading-relaxed">{{ toast.message }}</p>
          <button
