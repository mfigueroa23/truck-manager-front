  ...
  template: `
      ...
      <div class="relative">
        ...
        <input
          ...
          class="w-full pl-9 pr-4 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-sm text-slate-200
                 placeholder:text-slate-600 focus:outline-none focus:ring-2 focus:ring-orange-500/40 focus:border-orange-500/40 transition-all"
        />
      </div>
              ...
              @for (emp of filtradas(); track emp.id) {
                <tr class="hover:bg-slate-800/40 transition-colors group">
                  <td class="px-5 py-3.5">
                    <span class="text-xs font-mono text-slate-600">#{{ emp.id }}</span>
                  </td>
                  <td class="px-5 py-3.5">
                    <span class="font-medium text-slate-200">{{ emp.nombre }}</span>
                  </td>
                  <td class="px-5 py-3.5">
    ...
    >
      <form (ngSubmit)="guardar()" class="space-y-4">
        <div>
          <label class="block text-xs font-medium text-slate-400 mb-1.5">Nombre de la empresa</label>
          <input
            type="text"
            [(ngModel)]="form.nombre"
            name="nombre"
